...
function program( tokens ){
    ...
    while( true ){
        if( tokens.length ){
            unshift( body, expressionStatement() );
        } else {
...
export default function AST( tokens ){
    ...
    this.column = this.tokens.source.length;
    this.line = 1;

    this.program = program( this.tokens );
...
astPrototype.literal = function(){
    ...
    switch( token.type ){
        case Grammar.NumericLiteral:
            return new Node.NumericLiteral( raw );
        case Grammar.StringLiteral:
            return new Node.StringLiteral( raw );
        case Grammar.NullLiteral:
            return new Node.NullLiteral( raw );
        default:
