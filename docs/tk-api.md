...
**Note** The simple case keypaths (see `get`) should **not** be pre-compiled, since this will prevent `get` and `set` from being able to use the optimized code for that class of keypaths. If any operator character is present other than the separator (`.` by default, see `setOptions`), then the path is considered 'complex' and may benefit from pre-compilation. If none of these characters are present in the keypath then `getTokens` should **not** be used. The simple vs. complex test **does** account for custom separator characters, so it is safe to re-define the separator without sacrificing performance.
